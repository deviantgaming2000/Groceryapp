...
async function apiPost(path, body) {
    ...
    if (!res.ok)
        throw new Error(`API POST ${path} → ${res.status}: ${await res.text()}`);
    return res.json();
}
async function apiPatch(path, body) {
    ...
    if (!res.ok)
        throw new Error(`API PATCH ${path} → ${res.status}: ${await res.text()}`);
    return res.json();
}
...
async function flippSearch(query, postalCode) {
    ...
    if (!res.ok)
        throw new Error(`Flipp API → ${res.status}`);
    return res.json();
}
async function flippPublications(postalCode, merchantFilter) {
    const url = `${FLIPP_BASE}/flipp/publications?locale=en-us&postal_code=${encodeURIComponent(postalCode)}`;
    const res = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0", "Accept": "application/json" } });
    if (!res.ok)
        throw new Error(`Flipp publications API → ${res.status}`);
    const data = await res.json();
    const pubs = Array.isArray(data) ? data : (data.publications ?? data.flyers ?? []);
    if (merchantFilter) {
        return pubs.filter((p) => (p.merchant ?? p.name ?? "").toLowerCase().includes(merchantFilter.toLowerCase()));
    }
    return pubs;
}
// ── MCP Server ───────────────────────────────────────────────────────────────
const server = new Server({ name: "grocery-price-checker", version: "1.0.0" }, { capabilities: { tools: {} } });
const TOOLS = [
    // ── Dashboard ──
    {
    ...
    try {
        switch (name) {
            ...
            case "grocery_get_prices": {
                const params = new URLSearchParams();
                if (a.groceryItemId)
                    params.set("groceryItemId", a.groceryItemId);
                if (a.storeId)
                    params.set("storeId", a.storeId);
            ...
            case "grocery_get_lists": {
                ...
                const filtered = a.activeOnly ? lists.filter((l) => l.isActive) : lists;
                return ok(filtered);
            }
            ...
            case "grocery_list_coupons": {
                const coupons = await apiGet("/api/coupons");
                return ok(a.activeOnly ? coupons.filter((c) => c.isActive) : coupons);
            }
            ...
            case "flipp_search_deals": {
                ...
                if (!zip)
                    return err("No postal code provided. Pass postalCode or set FLIPP_POSTAL_CODE env var.");
                try {
                    const data = await flippSearch(a.query, zip);
                    const items = Array.isArray(data) ? data : (data.items ?? data.flyer_items ?? []);
                    if (!items.length)
                        return ok({ message: "No deals found on Flipp.", query: a.query, zip });
                    return ok(items.slice(0, 20).map((item) => ({
            ...
            case "flipp_get_store_flyer": {
                ...
                try {
                    ...
                    if (!pubs.length)
                        return ok({ message: `No Flipp flyer found for "${a.storeName}" near ${zip}.` });
                    return ok(pubs.slice(0, 5).map((p) => ({
                        id: p.id,
            ...
            case "flipp_import_deal": {
                ...
                const items = await apiGet(`/api/items?search=${encodeURIComponent(a.itemName)}`);
                let item = items.find((i) => i.name.toLowerCase().includes(a.itemName.toLowerCase()));
                if (!item) {
                ...
                const store = stores.find((s) => s.name.toLowerCase().includes(a.storeName.toLowerCase()));
                if (!store)
                    return err(`Store "${a.storeName}" not found. Add it first with grocery_create_store.`);
                const results = [];
